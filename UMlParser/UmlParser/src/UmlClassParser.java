...
public class UmlClassParser {
	...
	public void parse() throws Exception{
		...
		UmlDiagram.generatePNG(output, srcFolder+"/"+outputFile);
	}
    ...
    private void getDetails(CompilationUnit cu2) {
    					...
    					if(b instanceof MethodDeclaration) {
    								...
    								((MethodDeclaration) b).getName().toString().startsWith("set"))
    						{
	    						...
	    						for(Node i : methodBody){
                                        if (i.toString().contains("return")){
                                        	field=i.toString().substring(i.toString().lastIndexOf(" ") + 1);
                                        	System.out.println("Field is "+field);
                                        }
	    						}
    							String fieldname=((MethodDeclaration) b).getName().toString().substring(3);
    							replaceGetSet.add(fieldname.toLowerCase());
    							replaceGetSet.add(field.toLowerCase());
    						...
    						else
    						{
		    						...
		    						if (methodParameter.isEmpty()){
		    							...
		    							methodMapping+=":"+methodReturnType+";";
				    						...
				    						System.out.println("methodMapping is "+methodMapping);
		    						}
           ...
           // } 
        	...
        	for (String fieldname : replaceGetSet) {
        		System.out.println("Get set change"+fieldname);
        		System.out.println("attributeOutput "+attributeOutput);
